...
//À compléter :
/*@ predicate isMinPerm{L}(int *t, integer n) = \forall integer i ; 0 <= i < n - 1 ==> \at(t[i], L) < \at(t[i+1], L);

    predicate isMaxPerm{L}(int *t, integer n) = \forall integer i ; 0 <= i < n - 1 ==> \at(t[i], L) > \at(t[i+1], L);
 */
