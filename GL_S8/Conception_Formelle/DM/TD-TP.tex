        ...
        \begin{solutionorbox}
            ...
            \begin{enumerate}
                \item $0 \leq j \leq n-1$
                    Car j est initialize a $n-1$. Elle decremente par 1 a chaque tour de boucle jusqu'a la sortie de boucle, et, dans les pires des cas, nous avons $j == 0$, cas dans lequel nous n'avons pas trouve de j adequat.
                \item  $\forall$ integer $k ; 0 < k \leq n - 1 ==> P[k - 1] \geq P[k]$
                    Nous utilisons cet invariant car c'est simplement la deuxiemme condition d'arret de la boucle, ce qui doit donc etre valide dans chaque iteration de la boucle.
            \end{enumerate}
...
loop variant j;
        ...
        \begin{solutionorbox}
            ...
            \begin{enumerate}
                \item $i < j \leq n$
                    Car i est initialize a $j-1$, donc $i < j$. $j$ augmente de 1 a chaque tour de boucle jusqu'a la sortie de boucle, et, dans les pires des cas, nous avons $j == n$, cas dans lequel nous n'avons pas trouve de j adequat.
                \item  $\forall$ integer $k ; i < k < n ==> P[i] < P[k] $
                    Nous utilisons cet invariant car c'est simplement la deuxiemme condition d'arret de la boucle, ce qui doit donc etre valide dans chaque iteration de la boucle.
            \end{enumerate}
            La clause assign,ainsi que le variant choisi sont:
            \begin{lstlisting}
...
loop variant n - j;
            \end{lstlisting}
            Le choix du variant $n-j$ est car initialement nous avons $0 < j <= n$, et $j$ incremente a chaque tour de boucle, ce qui fait en sorte que $n-j$ decremente a chaque tour de boucle, car $n$ reste inchange.
        \end{solutionorbox}
