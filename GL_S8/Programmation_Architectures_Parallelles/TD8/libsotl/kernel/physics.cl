...
__kernel
void update_position(__global calc_t *pos, __global calc_t *speed,
...
{
    unsigned index = get_global_id(0);
    unsigned axis = index/offset;
    if(pos[index] < min[axis] || pos[index] > max[axis])
    {
...
{
    store3coord (speed, (coord_t)0.0, offset);
}
