...
int Machine::copyStringFromMachine(int from, char *to, unsigned size){
	...
	unsigned int i = 0;
	while(c != '\0' && i < size){
		// because of readMem corrupting 4 characters at the address given,
		// we need to use a temp variable to prevent readMem from corrupting
		// *to
		ReadMem(from + i, 1, &c);
		to[i] = c;
		i++;
...
int Machine::copyStringToMachine(int to, char* string, unsigned size){
	unsigned int i = 0;
	while(i < size){
		WriteMem(to + i, 1, string[i]);
		i++;
	}
	return i;
}
