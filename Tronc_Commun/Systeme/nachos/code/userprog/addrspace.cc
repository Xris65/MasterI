...
static void ReadAtVirtual(OpenFile *executable, int virtualaddr, int numBytes, int position, TranslationEntry *pagetable, unsigned numPage){
    TranslationEntry *tableSave = machine->currentPageTable;
    unsigned oldTableSize = machine->currentPageTableSize;
    machine->currentPageTable = pagetable;
    machine->currentPageTableSize = numPage;

    char tmp[numBytes];
    // char because we need to read one byte at a time
    executable->ReadAt(tmp, numBytes, position);
    for(int i = 0; i < numBytes; i++){
        machine->WriteMem(virtualaddr+i, 1, tmp[i]);
    }
    machine->currentPageTable = tableSave;
...
AddrSpace::AddrSpace (OpenFile * executable)
{
    ...
    threadCount = 1;
    currentThread->threadID = AllocateUserStack (); // pour le main
    #endif
...
int AddrSpace::AllocateUserStack(){
    return AddrSpace::bitmap->Find();
}
void AddrSpace::DeallocateUserStack(int index){
    AddrSpace::bitmap->Clear(index);
}
bool AddrSpace::SpaceAvailable(){
    return AddrSpace::bitmap->NumClear() > 0;
}
